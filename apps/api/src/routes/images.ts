...
// GET /images/camera/:camera_id - List images for a camera
images.get('/camera/:camera_id', requireAuth, async (c) => {
    try {
        const { camera_id } = c.req.param();
        const { limit = '50', offset = '0' } = c.req.query();

        const cameraImages = await sql`
            SELECT 
                i.id, i.file_path, i.thumbnail_path, 
                i.taken_at, i.uploaded_at,
                i.detected_animal, i.detected_animal_scientific,
                i.detection_confidence, i.confirmation_status,
                i.metadata,
